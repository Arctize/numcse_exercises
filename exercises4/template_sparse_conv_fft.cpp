...
template <class T> struct sparse_vec {
  ...
  void cleanup() {
    for (size_t i = 0; i < duplets.size(); i++) {
      if (duplets[i].ind >= len) {
	      duplets.erase
      }
    }
    sort(duplets.begin(), duplets.end());
    for (size_t i = 1; i < duplets.size(); ++i) {
      if (duplets[i].ind == duplets[i - 1].ind) {
        duplets[i - 1].val += duplets[i].val;
...
int main() {
  ...
  m.cleanup();
  cout << "TESTS. Correct componentwise multiplication between x and y: "
          "(1,(5,-10)),(3,(-23.184,11.44)),\n";
  cout << "cwise_mult(x,y) = " << m.to_string();

  auto c = sparse_vec<complex<double>>::conv(x, y);
  c.cleanup();
  cout << "Correct exact discrete convolution between x and y: "
          "(1,(41,0)),(2,(14.922,-42.8)),(3,(26.01,13.26)),(4,(-6.2,17.7)),(5,("
          "15.01,37.6386)),(6,(-7.184,16.28)),(7,(-9.6,16)),\n";
